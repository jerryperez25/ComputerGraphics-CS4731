...
var lightPos = [];
lightPos = vec4(0.0, 1.0, 50.0, 1.0);
var lightAmb = [];
...
spotlightang = 0;
var lightpos = [];
lightpos = vec4(0.0, 1.0, 30.0, 1.0 );
var howfar = [];
howfar = 4;
var parentNode;
parentNode = [];
    ...
    drawModels()
    {
        ...
        switch(arethereshadows)
        {
            case true:
            {
                ...
                var holder =  mult(viewMatrix, vec4(0.0, 0.0, 0.0, 1.0));
                var xcoor = holder[0];
                ypos = holder[1];
                ...
                if (xcoor < 0 )
                {
                    zpos = (shadowsize + xcoor) / ((xcoor / lightPos[2]) - 1);
                    xpos = -1 * zpos - shadowsize;
                }
                else if (xcoor == 0)
                {
                    zpos = -shadowsize;
                ...
                {
                    zpos = (xcoor - shadowsize) / ((xcoor / lightPos[2]) + 1);
                    xpos = zpos + shadowsize;
                }

                //function gets lost here, must replace with manual calls
                viewMatrix = translate(xpos, ypos-lightPos[1], zpos);
...
function main()
{
    ...
    // this is where we will set up our a,b,c, and d presses
    // A triggers shadows
    // B triggers texture
    // C triggers reflection
    // D triggers refraction
